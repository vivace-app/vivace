...
namespace Project.Scripts.StartupScreen
{
    public class StartupScreenProcessManager : MonoBehaviour
    {
        ...
        public bool continueButtonFlag = true; //trueのとき，Continueボタンを使用可能
        ...
        public void Update()
        {
            ...
            if (!_touchableFlag || Input.touchCount <= 0) return;
        ...
        private void LicenseCheck(string data)
        {
            ...
            var jsnData = JsonUtility.FromJson<LicenseResponse>(data);

            if (jsnData.success)
            {
                foreach (var x in jsnData.version)
                ...
                foreach (var x in jsnData.version)
                {
                    ...
                    if (ThisVersion != x.version) continue;
        ...
        {
            if (!continueButtonFlag) return;
            continueButtonFlag = false; //Continueボタンの連打を禁止
            StartCoroutine(isRecoveryMode ? RecoveryNetworkProcess() : RegisterNetworkProcess());
        }
        ...
        IEnumerator RegisterNetworkProcess()
        {
            if (inputUserName.text.Length >= 3 && inputUserName.text.Length <= 15)
            {
                ...
                switch (www.result)
                {
                    ...
                    case UnityWebRequest.Result.DataProcessingError:
                        ShowDialog(1, 0);
                        continueButtonFlag = true; //Continueボタンを再度有効化
                        break;
                    default:
        ...
        {
            var jsnData = JsonUtility.FromJson<RegisterResponse>(data);

            switch (jsnData.success)
            {
                case true:
                    showConnecting.text = "";
                    registerText.text = "";
        ...
        private IEnumerator RecoveryNetworkProcess()
        {
            ...
            var www = UnityWebRequest.Post(RecoveryApiUri, form);
        ...
        {
            var jsnData = JsonUtility.FromJson<RecoveryResponse>(data);
