 ...
 * <remarks>
 * <para header='説明'>CriWare.CriAtomExPlayer::Start 関数実行時に返されるオブジェクトです。<br/>
 * プレーヤ単位ではなく、 CriWare.CriAtomExPlayer::Start 関数で再生した個々の音声に対して
...
public struct CriAtomExPlayback
{
	 ...
	 * CriWare.CriAtomExPlayback::GetStatus 関数で取得可能です。<br/>
	 * <br/>
	 ...
	 * プレーヤで再生を行った（ CriWare.CriAtomExPlayer::Start 関数を実行した）
	 * 音声のステータスです。<br/>
	 * <br/>
	 ...
	 * 本関数ではなく CriWare.CriAtomExPlayer::Stop 関数をご利用ください。<br/>
	 * （ CriWare.CriAtomExPlayer::Stop 関数は、そのプレーヤで再生中の全ての音声を停止します。）<br/></para>
	 ...
	 * 本関数ではなく CriWare.CriAtomExPlayer::Pause 関数をご利用ください。<br/></para>
	 * </remarks>
	 ...
	 * <remarks>
	 * <para header='説明'>CriWare.CriAtomExPlayer::Start 関数で再生された音声のステータスを取得します。<br/></para>
	 * <para header='備考'>CriWare.CriAtomExPlayer::GetStatus 関数がAtomExプレーヤのステータスを返すのに対し、
	 ...
	 * - 再生中にエラーが発生した場合。
	 * .
	 * そのため、 CriWare.CriAtomExPlayback::Stop 関数を使用して明示的に再生を停止したか、
	 * その他の要因によって再生が停止されたかの違いに関係なく、
	 * 再生音のステータスはいずれの場合も Removed に遷移します。<br/>
	 * （エラーの発生を検知する必要がある場合には、本関数ではなく、 CriWare.CriAtomExPlayer::GetStatus
	 ...
	 * <remarks>
	 * <para header='説明'>CriWare.CriAtomExPlayer::Start 関数で再生された音声の再生時刻を取得します。<br/>
	 * <br/>
	 ...
	 * （ CriWare.CriAtomExPlayer::GetTime 関数と異なり、本関数は再生中の音声ごとに時刻を
	 ...
	 * 再生時刻が取得できた場合、本関数は 0 以上の値を返します。<br/>
	 * 指定したボイスが既に消去されている場合等には、本関数は負値を返します。<br/></para>
	 * <para header='備考'>CriWare.CriAtomExPlayback::GetTime 関数が返す「再生開始後からの経過時間」とは
	 ...
	 * <br/>
	 * 本関数を用いて再生時刻を取得するには、 CriWare.CriAtomExPlayer::CriAtomExPlayer(bool) 関数を
	 ...
	 * <remarks>
	 * <para header='説明'>CriWare.CriAtomExPlayer::Start 関数で再生された音声のシーケンス再生位置を取得します。<br/>
	 * <br/>
	 ...
	 * 本関数は負値を返します。<br/>
	 * <br/>
	 * CriWare.CriAtomExPlayer::Pause 関数でポーズをかけた場合、再生位置の更新も停止します。<br/>
	 ...
	 * <remarks>
	 * <para header='説明'>CriWare.CriAtomExPlayer::Start 関数で再生されたブロックシーケンスの、
	 ...
	 * <remarks>
	 * <para header='説明'>CriWare.CriAtomExPlayer::Start 関数で再生されたキューのトラック情報を取得します。<br/>
	 ...
	 * <returns>取得に成功したか</returns>
	 * <remarks>
	 * <para header='説明'>CriWare.CriAtomExPlayer::Start 関数で再生されたキューのビート同期情報を取得します。<br/>
	 ...
	 * <para header='備考'>再生開始ブロックの指定は CriWare.CriAtomExPlayer::SetFirstBlockIndex 関数を使用して行い、
	 ...
	 * <remarks>
	 * <para header='説明'>再生中のキューのビート同期オフセットを設定します。<br/>
	 * キューにはビート同期情報が設定されている必要があります。<br/>
	 * キューリンクやスタートアクションで再生しているキューの情報は取得できません。<br/></para>
	 * <para header='備考'>以下に該当するデータを再生中の場合、ビート同期オフセットの設定に失敗します。<br/>
	 * - キュー以外のデータを再生している。（ビート同期情報が存在しないため）<br/>
	 * - ビート同期情報が設定されていないキューを再生している。<br/>
	 * - ビート同期情報が設定されているキューを"間接的"に再生している。
	 *   （キューリンクやスタートアクションで再生している）<br/></para>
